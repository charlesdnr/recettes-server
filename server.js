...
if (!serviceAccountPath) {
  console.error(
    "FATAL ERROR: FIREBASE_KEY_PATH environment variable is not set."
  );
...
// --- Gestion "Session" Admin Simple (en mémoire) ---
// ATTENTION: Très basique, ne survit pas aux redémarrages serveur !
...
function isAdminTokenValid(token) {
  ...
  if (!isValid && currentAdminToken && Date.now() >= tokenExpiry) {
...
const adminAuthMiddleware = (req, res, next) => {
  ...
  if (isAdminTokenValid(token)) {
    ...
    res
      .status(401)
      .json({ message: "Accès non autorisé. Token invalide ou expiré." }); // Unauthorized
  }
};
// --- Fin Middleware ---
...
// Optionnel: Route pour vérifier si le token actuel est valide côté client
...
app.post(
  ...
  async (req, res) => {
    ...
    if (!bucket) {
      ...
      return res
        .status(500)
        .json({ message: "Erreur serveur: Storage non initialisé." });
    }
    console.log(
...
// POST /api/recipes
app.post("/api/recipes",adminAuthMiddleware, async (req, res) => {
  ...
  if (
    ...
    !rawRecipeData.imageUrl.startsWith("https://storage.googleapis.com/")
  ) {
    console.warn(
      "Recipe has an imageUrl but it doesn't seem to be a Firebase Storage URL."
    );
...
// PUT /api/recipes/:id
app.put("/api/recipes/:id",adminAuthMiddleware, async (req, res) => {
...
app.delete("/api/recipes/:id",adminAuthMiddleware, async (req, res) => {
  const recipeId = req.params.id;
  ...
  try {
    ...
    if (
      ...
      try {
        ...
        console.error(
          `Error deleting image ${imageUrlToDelete} from Firebase Storage (non-fatal):`,
          storageError
        );
      }
    } else {
      console.log(`Recipe ${recipeId} had no valid image URL to delete.`);
    }
...
app.get("/api/categories", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `GET /api/categories - Returning ${categoriesResult.length} categories from collection.`
    );
...
app.delete("/api/categories/:id",adminAuthMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (!recipesQuery.empty) {
      console.warn(
        `Attempted to delete category '${categoryName}' (ID: ${categoryId}) which is still in use by recipes.`
      );
      return res
        .status(409)
        .json({
          message: `Impossible de supprimer la catégorie '${categoryName}' car elle est utilisée par des recettes existantes.`,
        });
    }

    await categoryRef.delete();
    console.log(
      `Category '${categoryName}' (ID: ${categoryId}) deleted successfully.`
    );
    res.status(204).send();
  } catch (error) {
...
app.post("/api/categories/:id/subcategories",adminAuthMiddleware, async (req, res) => {
  const categoryId = req.params.id;
  ...
  try {
    ...
    if (alreadyExists) {
      return res
        .status(409)
        .json({
          message: `La sous-catégorie '${subcategoryName}' existe déjà dans cette catégorie.`,
        });
    }
    ...
    res
      .status(201)
      .json({
        message: `Sous-catégorie '${subcategoryName}' ajoutée avec succès.`,
      });
  } catch (error) {
    console.error(
...
app.delete("/api/categories/:id/subcategories/:name",adminAuthMiddleware, async (req, res) => {
  const categoryId = req.params.id;
  ...
  try {
    ...
    if (!recipesQuery.empty) {
      console.warn(
        `Attempted to delete subcategory '${subcategoryName}' from category '${categoryName}' (ID: ${categoryId}) which is still in use by recipes.`
      );
      return res
        .status(409)
        .json({
          message: `Impossible de supprimer la sous-catégorie '${subcategoryName}' car elle est utilisée par des recettes existantes.`,
        });
    }
    ...
    res
      .status(500)
      .json({
        message: "Erreur serveur lors de la suppression de la sous-catégorie.",
      });
  }
});
